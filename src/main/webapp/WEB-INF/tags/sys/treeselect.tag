...
<%@ attribute name="notAllowSelectChild" type="java.lang.Boolean" required="false" description="不允许选择子节点"%>
...
<div class="form-group form_search">
	...
	<input id="${id}Name" name="${labelName}" ${allowInput?'':'readonly="readonly"'} type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
		class="${cssClass}" style="${cssStyle}"/><a id="${id}Button" href="javascript:" class="btn btn_search ${disabled} ${hideBtn ? 'hide' : ''}" style="${smallBtn?'padding:4px 2px;':''}">&nbsp;<i class="icon-search"></i>&nbsp;</a>&nbsp;&nbsp;
</div>
				...
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
						...
						if (nodes[i].isParent){
							...
							return false;
						}//</c:if><c:if test="${notAllowSelectChild}">
						if (nodes[i].level == 2){
							top.$.jBox.tip("不能选择子节点（"+nodes[i].name+"）请重新选择。");
							return false;
