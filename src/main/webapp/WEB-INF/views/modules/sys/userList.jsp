...
<body>
	...
	<form:form id="searchForm" modelAttribute="user"
		...
		<sys:tableSort id="orderBy" name="orderBy" value="${page.orderBy}"
			callback="page();" />
		<ul class="ul-form">
			<li class="float_l"><label>归属区域：</label>
				<sys:treeselect id="company" name="company.id"
					value="${user.company.id}" labelName="company.name"
					labelValue="${user.company.name}" title="区域"
					url="/sys/office/treeData?type=1" cssClass="input-small input-sm"
					allowClear="true" />
			</li>
			<li><label>登录名：</label>
		...
		<ul class="ul-form btn_top_l">
			<li class="clearfix"></li>
			<li class="float_l"><label>归属学校：</label>
			<sys:treeselect id="office" name="office.id"
					value="${user.office.id}" labelName="office.name"
					labelValue="${user.office.name}" title="学校"
					url="/sys/office/treeData?type=2" cssClass="input-small input-sm"
		...
		<tbody>
			<c:forEach items="${page.list}" var="user">
				<tr>
					...
					<shiro:hasPermission name="sys:user:edit">
						<td><a href="${ctx}/sys/user/form?id=${user.id}" title="修改"><i class="icon-pencil"></i></a> <a
							href="${ctx}/sys/user/delete?id=${user.id}" title="删除"
							onclick="return confirmx('确认要删除该用户吗？', this.href)"><i class="icon-trash"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	<nav aria-label="Page navigation" class="navigation col-sm-12 col-m d-12 col-lg-12">${page}</nav>
</body>
</html>