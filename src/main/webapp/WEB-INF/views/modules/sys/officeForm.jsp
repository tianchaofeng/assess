...
<head>
	...
	function valid1(name){
	var code=null;
		var somecode=null;
		$.ajax({
			url:"${ctx}/mclms/recordCar/validOff",
			type:"POST",
			dataType:"JSON",
			data:{code:code,name:name,somecode:somecode},
			success:function(data){ 
				  if (data) {
					  $("#span_nameId").text("机构名称已经存在，请重新输入！"); 
					  var bit=document.getElementById("btnSubmit");
					  bit.disabled=true;
                }else{
                	$("#span_nameId").text(""); 
                	 var bit=document.getElementById("btnSubmit");
	 ...
	 function valid2(code){
		...
		$.ajax({
			...
			success:function(data){ 
				  if (data) {
					  $("#span_codeId").text("机构编码已经存在，请重新输入！");
					  var bit=document.getElementById("btnSubmit");
					  bit.disabled=true;
                }else{
					  $("#span_codeId").text("");
	...
	function valid3(somecode){
		...
		$.ajax({
			...
			success:function(data){ 
				  if (data) {
					  $("#span_somecode").text("机构内码已经存在，请重新输入！");
					  var bit=document.getElementById("btnSubmit");
					  bit.disabled=true;
                }else{
					  $("#span_somecode").text("");
...
<body>
	...
	<form:form id="inputForm" modelAttribute="office" action="${ctx}/sys/office/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="name" id="nameId" htmlEscape="false" onchange="valid1(this.value)" maxlength="50" class="required"/>
				<span class="help-inline" id="span_nameId" style="color: red"></span>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<%-- <div class="control-group">
			...
			<div class="controls">
				<form:input path="code" id="code" htmlEscape="false" onchange="valid2(this.value)" maxlength="50"/>
				<span class="help-inline" id="span_codeId" style="color: red"></span>
			</div>
		</div> --%>
		<%-- <div class="control-group">
			<label class="control-label">机构内码:</label>
			<div class="controls">
				<form:input path="somecode" id="somecode" onchange="valid3(this.value)" htmlEscape="false" maxlength="50"/>
				<span class="help-inline" id="span_somecode" style="color: red"></span>
			</div>
		</div> --%>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="useable">
					<form:options items="${fns:getDictList('sys_office_useable')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
