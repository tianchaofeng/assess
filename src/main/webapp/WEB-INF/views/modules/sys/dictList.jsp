		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<shiro:hasPermission name="sys:dict:edit"><td>
    				<a href="${ctx}/sys/dict/form?id=${dict.id}" title="修改"><i class="icon-pencil"></i></a>
					<a href="${ctx}/sys/dict/delete?id=${dict.id}&type=${dict.type}" title="删除" onclick="return confirmx('确认要删除该字典吗？', this.href)"><i class="icon-trash"></i></a>
    				<a title="增加" href="<c:url value='${fns:getAdminPath()}/sys/dict/form?type=${dict.type}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>"><i class=" icon-plus-sign-alt"></i></a>
