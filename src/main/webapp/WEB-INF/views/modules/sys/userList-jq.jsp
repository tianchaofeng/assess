...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/listData" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
			<li><label>归属区域：</label><sys:treeselect id="company" name="company.id" value="${user.company.id}" labelName="company.name" labelValue="${user.company.name}" 
				title="区域" url="/sys/office/treeData?type=1" cssClass="input-small input-sm" allowClear="true"/></li>
			<li><label>登录名：</label><form:input path="loginName" htmlEscape="false" maxlength="50" class="input-medium"/></li>
			<li class="clearfix"></li>
			<li><label>归属学校：</label><sys:treeselect id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}" 
				title="学校" url="/sys/office/treeData?type=2" cssClass="input-small input-sm" allowClear="true" notAllowSelectParent="true"/></li>
	...
	<%-- <table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>归属区域</th><th>归属学校</th><th class="sort-column login_name">登录名</th><th class="sort-column name">姓名</th><th>电话</th><th>手机</th><th>角色</th><shiro:hasPermission name="sys:user:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
	...
	<nav aria-label="Page navigation" class="navigation">${page}</nav> --%>
	...
	<script type="text/javascript">
		...
		$('#dataGrid').dataGrid({
			...
			// 设置数据表格列
			columnModel: [
				{header:'归属区域', name:'loginName', index:'login_name', width:100, frozen:true , formatter: function(val, obj, row, act){
					return '<a href="${ctx}/sys/user/form?id='+row.id+'" class="btnList" data-title="编辑用户">'+val+'</a>';
				}},
				{header:'归属区域', name:'company.name', index:'', width:160},
