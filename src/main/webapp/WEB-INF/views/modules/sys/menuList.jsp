			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a href="${ctx}/sys/menu/form?id=${menu.id}" title="修改"><i class="icon-pencil" ></i></a>
						<a href="${ctx}/sys/menu/delete?id=${menu.id}" title="删除" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)"><i class="icon-trash"></i></a>
						<a href="${ctx}/sys/menu/form?parent.id=${menu.id}" title="增加"><i class="icon-plus-sign-alt"></i></a> 
