		...
		<c:forEach items="${list}" var="role">
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/sys/role/assign?id=${role.id}" title="分配"><i class="icon-sitemap"></i></a>
					<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
						<a href="${ctx}/sys/role/form?id=${role.id}" title="修改"><i class="icon-pencil"></i></a>
					</c:if>
					<a href="${ctx}/sys/role/delete?id=${role.id}" title="删除" onclick="return confirmx('确认要删除该角色吗？', this.href)"><i class="icon-trash"></i></a>
