	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="sys:area:edit"><td>
				<a href="${ctx}/sys/area/form?id={{row.id}}" title="修改"><i class="icon-pencil"></i></a>
				<a href="${ctx}/sys/area/delete?id={{row.id}}" title="删除" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)"><i class="icon-trash"></i></a>
				<a href="${ctx}/sys/area/form?parent.id={{row.id}}" title="增加"><i class="icon-plus-sign-alt"></i></a> 
