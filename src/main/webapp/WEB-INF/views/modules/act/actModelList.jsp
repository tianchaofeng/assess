		...
		<tbody>
			<c:forEach items="${page.list}" var="model">
				<tr>
					...
					<td>
						<a href="${pageContext.request.contextPath}/act/rest/service/editor?id=${model.id}" target="_blank" title="修改"><i class="icon-pencil"></i></a>
						<a href="${ctx}/act/model/deploy?id=${model.id}" onclick="return confirmx('确认要部署该模型吗？', this.href)" title="部署"><i class="icon-cog"></i></a>
						<a href="${ctx}/act/model/export?id=${model.id}" target="_blank" title="导出"><i class="icon-share-alt"></i></a>
	                    <a href="${ctx}/act/model/delete?id=${model.id}" onclick="return confirmx('确认要删除该模型吗？', this.href)" title="删除"><i class="icon-trash"></i></a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
